...
def _parse_date(raw: str) -> str:
    ...
    # Last resort: today
    logger.info(f"Could not parse date '{raw}' — using today.")
...
def _parse_flights_with_gemini(
    ...
    try:
        ...
        text     = re.sub(r"```(?:json)?", "", response["message"]["content"]).strip().rstrip("`").strip()
...
def flight_finder(parameters: dict, player=None, speak=None) -> str:
    ...
    try:
        ...
        if not raw_text:
            return "Could not retrimia flight data, sir. The page may not have loaded."
