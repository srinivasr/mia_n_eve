...
try:
    ...
    notification.notify(title="MIA Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("MIA Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "MIA Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by MIA reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>MIA Reminder</Description></RegistrationInfo>\n'
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.mia.reminder.{task_name}"
...
"""
    ...
    if result.returncode != 0:
        ...
        script_path.unlink(missing_ok=True)
        logger.info(f"launchctl: {result.stderr.strip()}")
        return ""
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        logger.info(f"systemd-run failed: {result.stderr.strip()}, trying 'at'")
    ...
    if shutil.which("at"):
        ...
        if result.returncode == 0:
            return task_name
        logger.info(f"at: {result.stderr.strip()}")
        return ""

    logger.info("Neither systemd-run nor at found on this Linux system.")
    return ""
...
def reminder(
    ...
    task_name  = f"MIAReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        logger.info(f"Scheduling exception: {e}")
