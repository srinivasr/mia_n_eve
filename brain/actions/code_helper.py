        ...
        def generate_content(self, contents):
            ...
            class DummyResp:
                def __init__(self, text):
                    self.text = text
            resp = self.client.chat(model=get_ollama_model(), messages=[{"role": "user", "content": contents}])
            return DummyResp(resp["message"]["content"])
            
    return ModelWrapper(_CLIENT)
...
def _write(description: str, language: str, output_path: str, player=None) -> tuple[str, Path]:
    ...
    prompt = f"""You are an expert {lang} dmialoper.
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        logger.info(f"Attempt {attempt}/{MAX_BUILD_ATTEMPTS}")
        if player:
        ...
        logger.info(f"Error on attempt {attempt}, fixing...")
        if player:
...
def _optimize_action(file_path, code, language, output_path, player) -> str:
    ...
    prompt = f"""You are an expert {lang} dmialoper and code reviewer.
...
Optimized code:"""
    ...
    logger.info(f"Optimized: {save_path}")
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    if file_path:
        ...
        if err:
            logger.info(f"Could not read file: {err}")
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        if file_path and file_content:
            ...
            if code_match:
                ...
                logger.info(f"Fixed code saved: {file_path}")
...
def code_helper(
    ...
    if action == "auto":
        ...
        logger.info(f"Auto-detected: {action}")
