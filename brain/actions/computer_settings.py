...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            logger.info(f"pycaw failed, using keypress fallback: {e}")
            pyautogui.press("volumemute")
            pyautogui.press("volumemute")
    elif _OS == "Darwin":
    ...
    if _OS == "Darwin":
        ...
        if subprocess.run(["which", "brightnessctl"],
            ...
            subprocess.run(
                ...
                '.split(\"Brightness:\")[1].split()[0]); logger.info(min(1.0,b+0.1))")',
        ...
        try:
            ...
            logger.info(f"Brightness up failed on Windows: {e}")
    ...
    if _OS == "Darwin":
        ...
        if subprocess.run(["which", "brightnessctl"],
            ...
            subprocess.run(
                ...
                ' --brightness $(python3 -c "import subprocess; '
                'b=float(subprocess.check_output([\"xrandr\",\"--verbose\"]).decode()'
                '.split(\"Brightness:\")[1].split()[0]); logger.info(max(0.1,b-0.1))")',
                shell=True, capture_output=True
            )
    else:
        try:
            ...
            logger.info(f"Brightness down failed on Windows: {e}")
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        ...
        resp = client.chat(model=get_ollama_model(), messages=messages)
        text = re.sub(r"```(?:json)?", "", resp["message"]["content"]).strip().rstrip("`").strip()
...
def computer_settings(
    ...
    logger.info(f"Action: {action}  Value: {value}  OS: {_OS}")
    if player:
    ...
    try:
        ...
        logger.info(f"Action failed ({action}): {e}")
