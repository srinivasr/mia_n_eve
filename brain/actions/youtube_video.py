...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        url = simpledialog.askstring("M.I.A.", prompt_text, parent=root)
...
def _summarize_with_gemini(transcript: str, video_url: str) -> str:
    ...
    if _CLIENT is None:
        ...
        _CLIENT = ollama.Client(host=get_ollama_url())
    
    client = _CLIENT
    
    system_instruction = (
        "You are MIA, an AI assistant. "
    ...
    response = client.chat(model=get_ollama_model(), messages=messages)
...
def _save_summary(content: str, video_url: str) -> str:
    ...
    try:
        ...
        logger.info(f"Could not open text editor: {e}")
...
def _handle_play(parameters: dict, player) -> str:
    ...
    logger.info(f"Scraping first non-Shorts video for: {query}")
    ...
    logger.info("Scrape failed, opening filtered search page")
    fallback_url = (
        f"https://www.youtube.com/results"
        f"?search_query={quote_plus(query)}"
        f"&sp={_YT_VIDEO_FILTER}"
...
def _handle_summarize(parameters: dict, player, speak) -> str:
    ...
    if not transcript:
        return "I couldn't retrimia a transcript for that video, sir."

    if speak:
        speak("Transcript retrimiad. Generating summary now.")
...
def _handle_get_info(parameters: dict, player, speak) -> str:
    ...
    if not info:
        return "Could not retrimia video information, sir."
...
def youtube_video(
    ...
    try:
        ...
        logger.info(f"Error in {action}: {e}")
