...
- Handle promise rejections with try/catch in async functions."""
    ...
    prompt = f"""You are a senior {language} dmialoper writing production-quality code for a real project.
...
Code for {file_path}:"""
    ...
    try:
        ...
        logger.info(f"Written: {file_path} ({len(code)} chars)")
        return code
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    for cmd in vscode_candidates:
        try:
            ...
            time.sleep(1.5)
            logger.info(f"VSCode opened: {project_dir}")
            return True
        except Exception:
            continue
    return False
...
def _try_auto_install(error_output: str, project_dir: Path) -> bool:
    ...
    logger.info(f"Auto-installing missing package: {pkg}")
    try:
        result = subprocess.run(
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            logger.info(f"Fixed: {fix_path}")

        except Exception as e:
            if _is_rate_limit(e):
                raise RateLimitError(str(e))
            logger.info(f"Could not fix {fix_path}: {e}")
