...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".mia_profiles" / browser
    fallback.mkdir(parents=True, exist_ok=True)
    logger.info(f"Real profile not found for {browser}, using: {fallback}")
    return str(fallback)
...
def _firefox_profile_dir() -> Optional[str]:
    ...
    if default_path and Path(default_path).exists():
        logger.info(f"Firefox real profile: {default_path}")
        return default_path
    return None
...
def _find_opera_windows() -> Optional[str]:
    ...
    for p in candidates:
        if p.exists():
            logger.info(f"Opera found at: {p}")
            return str(p)
            ...
            for hive in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
                try:
                    ...
                    if exe and Path(exe).exists():
                        logger.info(f"Opera found via registry: {exe}")
                        return exe
                except Exception:
                    continue
    except Exception:
...
def _resolve_browser(name: str) -> dict | None:
    ...
    if spec.get("special") == "opera_windows":
        ...
        if not exe:
            logger.info("Opera executable not found on Windows.")
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".mia_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                logger.info(f"Firefox real profile failed ({e}), using MIA profile")
                mia = str(Path.home() / ".mia_profiles" / "firefox_mia")
                Path(mia).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(mia, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".mia_profiles" / "safari")
        ...
        try:
            ...
            logger.info(f"Real profile failed for {label}: {e}")

        mia_profile = str(Path.home() / ".mia_profiles" / self.browser_name)
        Path(mia_profile).mkdir(parents=True, exist_ok=True)
        logger.info(f"Retrying with MIA profile: {mia_profile}")
        
        try:
            self._context = await engine_obj.launch_persistent_context(mia_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            logger.info(f"Launched [{label}] with MIA profile")
    ...
    def _get_or_create(self, browser_name: str) -> _BrowserSession:
        with self._lock:
            if browser_name not in self._sessions:
                ...
                logger.info(f"New session: {browser_name}")
