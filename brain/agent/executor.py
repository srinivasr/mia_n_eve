...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    try:
        ...
        logger.info(f"Running generated code: {tmp_path}")
    ...
    if tool == "file_controller" and params.get("action") in ("write", "create_file"):
        ...
        if not content or len(content) < 50:
            ...
            if all_results:
                ...
                logger.info(f"Injected + translated content")
...
def _translate_to_goal_language(content: str, goal: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"IMPORTANT:\n"
            f"- Translate MIARYTHING, leave nothing in English\n"
        ...
        logger.info(f"Translation done ({target_lang})")
        return translated
    except Exception as e:
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        ...
        logger.info(f"Unknown tool '{tool}' — falling back to generated_code")
            ...
            for step in steps:
                ...
                while attempt <= 3:
                    ...
                    try:
                        ...
                        error_msg = str(e)
                        logger.info(f"Step {step_num} attempt {attempt} failed: {error_msg}")
                        ...
                        if decision == ErrorDecision.RETRY:
                            ...
                            logger.info(f"Skipping step {step_num}")
                            completed_steps.append(step)
                            step_ok = True
                            break
                            ...
                            if fix_suggestion and tool != "generated_code":
                                try:
                                    ...
                                    logger.info(f"Fix failed: {fix_err}")
