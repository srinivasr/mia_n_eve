...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not win:
            logger.info("Steam penceresi bulunamadı")
            return False
        ...
        if not colorful.any():
            logger.info("Avatar rengi bulunamadı — tahminle tıklanıyor")
        ...
        logger.info(f"Profil avatarı ({abs_x}, {abs_y}) — tıklanıyor")
        pyautogui.click(abs_x, abs_y)
        return True

    except ImportError as e:
        logger.info(f"Eksik kütüphane: {e}")
        return False
    except Exception as e:
        logger.info(f"Profil tespiti başarısız: {e}")
        return False


def _handle_steam_profile_selection() -> bool:
    logger.info("Profil seçim dialogu kontrol ediliyor...")
    win = _get_steam_window_rect()
    if not win:
        return False
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            logger.info("Profil dialogu yok — Steam zaten giriş yapmış")
            return False
    except ImportError:
        pass
    except Exception:
    ...
    logger.info("Profil seçimi tespit edildi — ilk profile tıklanıyor")
        ...
        try:
            for ctrl in dialog.descendants(control_type=control_type):
                if target in ctrl.window_text().upper():
                    ctrl.click_input()
                    logger.info(f"Sürücü seçildi ({control_type}): {ctrl.window_text()}")
                    return True
        except Exception:
            continue
    try:
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    logger.info(f"Hedef sürücü: {drive_label} ({best_drive['free_gb']:.1f} GB boş)")
    ...
    try:
        ...
        logger.info(f"pywinauto başarısız: {e}")
        return _handle_install_dialog_pyautogui(game_name, best_drive)
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    if not exe.exists():
        logger.info(f"Steam bulunamadı: {exe}")
        return False

    logger.info("Steam başlatılıyor...")
    if is_mac():
    ...
    logger.info("Steam başlatılamadı")
    return False
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    if name_lower in _KNOWN_APPIDS:
        ...
        logger.info(f"Bilinen: {canonical} ({app_id})")
        return app_id, canonical

    for key, (app_id, canonical) in _KNOWN_APPIDS.items():
        if name_lower in key or key in name_lower:
            logger.info(f"Kısmi eşleşme: {canonical} ({app_id})")
            return app_id, canonical
    ...
    try:
        ...
        if items:
            best = items[0]
            logger.info(f"Store API: {best['name']} ({best['id']})")
            return str(best["id"]), best["name"]
    except Exception as e:
        logger.info(f"AppID arama başarısız: {e}")
...
def _install_steam_game(steam_path: Path, game_name: str = None,
    ...
    if not app_id and game_name:
        ...
        logger.info(f"Kuruluyor: {game_name} (AppID: {app_id})")
...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.mia.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.mia.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "MIA_GameUpdater", "/F"],
            capture_output=True, text=True
        )
        return ("Scheduled update cancelled."
                if result.returncode == 0 else "No scheduled update found.")
    if is_mac():
        plist_path = Path.home() / "Library" / "LaunchAgents" / "com.mia.gameupdater.plist"
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "MIA_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True
        )
        if result.returncode != 0:
...
if __name__ == "__main__":
    if "--scheduled" in sys.argv:
        logger.info(f"Scheduled run at {datetime.now().strftime('%H:%M')}")
